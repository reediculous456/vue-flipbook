...
(function($) {
  ...
  const turnMethods = {
    ...
    zoom(newZoom) {
      ...
      if (typeof newZoom == `number`) {
        ...
        for (let i = 0; i < currentView.length; i += 1) {
    ...
    _movePages(from, change) {
      ...
      if (change > 0)
      { for (page = data.totalPages; page >= from; page -= 1)
      { move(page); } }
      else
      { for (page = from; page <= data.totalPages; page += 1)
      { move(page); } }
    ...
    resize() {
      ...
      for (page = 1; page <= data.totalPages; page += 1)
      { if (data.pages[page])
    ...
    _removeMv(page) {
      ...
      for (i = 0; i < data.pageMv.length; i += 1)
    ...
    pages(pages) {
      ...
      if (pages) {
        ...
        if (pages < data.totalPages) {
          ...
          for (let page = data.totalPages; page > pages; page -= 1)
    ...
    _fitPage(page) {
      ...
      for (let i = 0; i < newView.length; i += 1) {
    ...
    _updateShadow() {
      ...
      for (let i = 0; i < data.pageMv.length; i += 1) {
        if (!view[0] || !view[1])
        { break; }
      ...
      switch (data.effect) {
        ...
        case `hard`:
          ...
          if (turnData.opts.gradients) {
            ...
            } else if (data.bshadow.parent()[0] != data.fpage[0]) {
        ...
        case `sheet`:
          ...
          const ac = turnData.opts.acceleration;
          const h = data.wrapper.height();
        ...
        if (!visible) {
          ...
          switch (data.effect) {
            ...
            case `sheet`:
              ...
              data.visible = true;
              data.fparent.show().data().flips += 1;
              flipMethods._moveFoldingPage.call(this, true);
              data.fwrapper.show();
              if (data.bshadow)
              { data.bshadow.show(); }
  ...
  function rotationAvailable() {
    ...
    if (parts = /AppleWebkit\/([0-9.]+)/i.exec(navigator.userAgent)) {
      const webkitVersion = parseFloat(parts[1]);
